...
class splitInDepth(nn.Module): pass
class splitInLayers(nn.Module): pass
...
class Encoder(nn.Module): 
    ...
    """
    def __init__(self, unit, embeddingModel):
        super().__init__()
        self.unit = unit
    ...
    def forward(self,   seq:              TensorType['batch_size', 'seq_len', 'embed_size',int], 
        ...
        seq = self.embeddingModel(seq)
        seq_hiddens, last_state = self.unit(seq, init_state) #Aplico el encoder Bi-LSTM
