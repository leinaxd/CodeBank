...
class Decoder(nn.Module):
    """A simple LSTM decoder"""
    def __init__(self, unit, embedding, projection, startToken, endToken, maxDecodingTime=100):
        super().__init__()
        self.decoderUnit = unit
        self.embedding = embedding
        self.startToken = startToken #SOS
        self.endToken = endToken #EOS
        self.maxDecodingTime = maxDecodingTime
        self.projectionLayer = projection
    def forward(self,
                dec_init_state: Tuple[ TensorType["nLayers","batch_size","dec_hidden_size",float], 
                                       TensorType["nLayers","batch_size","dec_hidden_size",float]], 
                ) ->            TensorType['tgt_len','batch_size','tgt_vocab', int]:
        nLayers = dec_init_state[0].shape[0]
        batch_size = dec_init_state[0].shape[1]
        embed_size = dec_init_state[0].shape[2]

        symbol     = torch.zeros([batch_size, 1], dtype=int).fill_(self.startToken)
        predictions = []
        decoding_time  = 0
        tgt_len = torch.zeros(batch_size, dtype=int)
        flags    = torch.tensor([False]*batch_size) #Flags until reach <end>

        while decoding_time < self.maxDecodingTime:
            decoding_time += 1
            dec_input = self.embedding(symbol)
            dec_out, dec_init_state = self.decoderUnit(dec_input, dec_init_state)
            out = self.projectionLayer(dec_out) #output_size
            predictions.append(out) #[batch_size x vocab, ...]
            symbol = self.greedyStrategy(out)
            #Break when prediction has all <EOS>
            ixEOS = self.endToken == symbol.squeeze(-1)
        ...
        predictions = torch.stack(predictions,1) #concatenate through tgt_len dim
        predictions.squeeze_(-2) #squeeze layer dimension
        logP = F.log_softmax(predictions, dim=-1) #ouput = probability of each word
        return logP, tgt_len

    def greedyStrategy(self, dec_out:TensorType['batch_size']) -> TensorType['batch_size']:
        symbols = torch.argmax(dec_out,-1)
        return symbols
...
# class myDecodero(generativeModel):
