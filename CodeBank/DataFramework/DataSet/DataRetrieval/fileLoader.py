...
class FileLoader(Dataset):
    ...
    """
    def __init__(self, path:str, use_ram=True, trim_K:int=-1, show_progress=True):
        super().__init__()
        ...
        with open(path, 'rt') as f:
            if show_progress: progress_bar = tqdm(total   = os.path.getsize(path), 
                                                  desc    = f"reading {os.path.basename(path)}")
            while True:
                pos = f.tell()
                line = f.readline()
                if show_progress: progress_bar.update(len(line))
...
if __name__ == '__main__':
    trim_k = 6
    trim_k = -1
    loader_ram = FileLoader(__file__, trim_K=trim_k)
    loader_disk = FileLoader(__file__, False, trim_K=trim_k)
