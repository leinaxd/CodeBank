#intuitive Explanation
#   https://www.youtube.com/watch?v=c1XL5BeI9_s
#   - Haar wavelet: Guarda la media entre dos puntos, y la diferencia del primero
#   - A medida que me muevo al espacio de "detalle" (como voy computando las diferencias) los valores convergen a cero
#   - La reconstrucción Es una especie de fractal controlado

#   https://www.youtube.com/watch?v=fxfS0vSAsTA
#   - Wavelet como banco de filtros
#   - JBB determina la entropía del padre vs los hijos si conviene dividir la señal o no
#   - Una banda representa un feature. La idea es representar una señal en features de potencia.